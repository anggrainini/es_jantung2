...
class Home extends CI_Controller {
	...
	public function index(){
		$data['title']="Home";
		$inside['content']=$this->load->view('global/home',$data,TRUE);
		$this->load->view('global/frontend',$inside);
		
	}	

	public function about(){
		$data['title']="About";
		$inside['content']=$this->load->view('global/about',$data,TRUE);
		$this->load->view('global/frontend',$inside);
		
	}	

	public function try_pemeriksaan(){
		...
		$this->form_validation->set_rules('nama', 'nama', 'required');		
				if($this->form_validation->run()==FALSE)
				{
					$data['title']="Try";
					$data['hasil']='';
					$inside['content']=$this->load->view('global/try',$data,TRUE);
					$this->load->view('global/frontend',$inside);
				}
			 	else
				{
					$id = 'guest';
					$jk=$this->input->post('var_gender');
					...
					//create id_guest
					$this->Simple_CRUD->insertData('tb_login',$guest);
					$this->Simple_CRUD->insertData('tb_user',$guest);
					$query=$this->Simple_CRUD->insertData_get_id('tb_periksa',$data_clean);
					$data['periksa'] = $this->Simple_CRUD->get_detail1("tb_periksa","id_periksa",$query);
					if($data['periksa']['valid']=='1'){
						$hasil['hasil']=$this->proses_periksa($query);
					}else{
						$hasil['hasil']='Tidak dapat menampilkan hasil pemeriksaan, data tidak lengkap';
					}
					$this->Simple_CRUD->deleteData('tb_login',$guest);

					$data['title']="Try";
					$inside['content']=$this->load->view('global/try',$hasil,TRUE);
					$this->load->view('global/frontend',$inside);
	...
	private function proses_periksa($id){
